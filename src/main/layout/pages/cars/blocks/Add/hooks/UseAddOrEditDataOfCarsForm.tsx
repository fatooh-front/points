...
Props) {
  ...
  const { data: DataOfcar, isLoading } = useGetCars(id);
  ...
  const formSchema = z.object({
    ...
    detailsEnglish: z.string({ message: "تفاصيل السيارة مطلوبة" }).min(1, {
      message: "تفاصيل السيارة مطلوبة",
    }),
    detailsArabic: z.string({ message: "تفاصيل السيارة مطلوبة" }).min(1, {
      message: "تفاصيل السيارة مطلوبة",
    }),
  ...
  console.log(DataOfcar?.data?.images, "asdsadsadsadsad");
  ...
  useEffect(() => {
    if (DataOfcar?.data) {
      DataOfcar?.data.carStatus
        ? navigate(`/cars/edit/${DataOfcar?.data.carId}`)
        : null;
      form.reset(defaultValues);
      setStep(DataOfcar?.data.carId && !DataOfcar?.data.dailyPrice ? "2" : "3");
    }
  }, [DataOfcar, DataOfCarBranchbyCar, form]);
  useEffect(() => {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (step !== "3") {
      console.log("dffdgdfgfdgdfgdf1");

      mutateAdd(
        { ...restValues },
        {
          onSuccess: (res: any) => {
            navigate(`/cars/add/${res.data}`);
            // form.setValue("carId", res.data);
            // console.log("DataOfcar?.data edited successfully", res.data);
            // const stepNum = Number(step);
            // if (stepNum <= 2 && setStep) setStep(String(stepNum + 1));
            // Fetch the car by the returned id and update state
          },
          onError,
        }
      );
    } else {
      console.log("dffdgdfgfdgdfgdf3");
      ...
      mutateAddCarOnbranchs(
        ...
        {
          onSuccess: () => {
            form.setValue("carStatus", 1);
            refetchDataOfCarBranchbyCar();
            mutateAdd(
              { ...restValues, carStatus: 1 },
              {
                onSuccess: () => {
                  navigate(`/cars`);
                },
