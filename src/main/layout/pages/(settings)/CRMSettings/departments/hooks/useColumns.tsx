...
import { Department } from "@/main/global/api/restful/userManagmentAPI/CRMSettingsManager/CRMSettingsTypes";
import { useDeleteDepartment } from "@/main/global/api/restful/userManagmentAPI/CRMSettingsManager/CRMSettingsQuery";

export const useColumns = () => {
  const { mutate: mutate } = useDeleteDepartment();
  const columns: ColumnDef<Department>[] = [
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title="اسم القسم بالعربي" />
      ),
      cell: ({ row }) => row.original.arabicName,
      meta: {
        header: "اسم القسم بالعربي",
      },
    },
