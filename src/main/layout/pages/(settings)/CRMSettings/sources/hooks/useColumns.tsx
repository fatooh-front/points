...
import { TicketReason } from "@/main/global/api/restful/userManagmentAPI/CRMSettingsManager/CRMSettingsTypes";
import { useDeleteTicketReason } from "@/main/global/api/restful/userManagmentAPI/CRMSettingsManager/CRMSettingsQuery";

export const useColumns = () => {
  const { mutate: mutate } = useDeleteTicketReason();
  const columns: ColumnDef<TicketReason>[] = [
    {
      id: "englishName",
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title="اسم السبب بالعربي" />
      ),
      cell: ({ row }) => row.original.arabicName,
      meta: {
        header: "اسم السبب بالعربي",
      },
    },
