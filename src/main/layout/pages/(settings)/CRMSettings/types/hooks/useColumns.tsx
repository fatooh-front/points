...
import { TicketType } from "@/main/global/api/restful/userManagmentAPI/CRMSettingsManager/CRMSettingsTypes";
import { useDeleteTicketType } from "@/main/global/api/restful/userManagmentAPI/CRMSettingsManager/CRMSettingsQuery";

export const useColumns = () => {
  const { mutate: mutate } = useDeleteTicketType();
  const columns: ColumnDef<TicketType>[] = [
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title="اسم نوع دعم  بالعربي" />
      ),
      cell: ({ row }) => row.original.arabicName,
      meta: {
        header: "اسم نوع دعم  بالعربي",
      },
    },
