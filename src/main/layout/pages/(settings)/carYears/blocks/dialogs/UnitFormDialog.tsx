...
export function UnitFormDialog({ unit, type, btn = true }: Props) {
  ...
  const { i18n, t } = useTranslation("carsYear");
      ...
      {type !== "delete" && (
        <Dialog open={open} onOpenChange={setOpen} modal={false}>
          <DialogTrigger asChild>
            <TTriggerForm
              type={type}
              addText={t("carsYear.form.addText")}
              btn={btn}
            />
          </DialogTrigger>
          <DialogContent
                ...
                <Form {...form}>
                  ...
                  >
                    <div className="flex flex-col md:flex-1 gap-y-3 ">
                      <TFormField
                        placeholder={t("carsYear.form.placeholder.name_en")}
                        typeField="input"
                        form={form}
                        name="year"
                        label={t("carsYear.form.year")}
                        readOnly={type === "view"}
                      />{" "}
                      <div className=" mt-[24px]">
                        {/* <TextEditor
                          form={form}
                          name="notes"
                          placeholder={t("carsYear.form.placeholder.notes")}
                          label={t("carsYear.form.notes")}
                          readOnly={type === "view"}
                        /> */}
                      </div>
                    </div>
