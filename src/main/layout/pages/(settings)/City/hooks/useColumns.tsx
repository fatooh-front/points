import { ColumnDef } from "@tanstack/react-table";
import { useTranslation } from "react-i18next";

import { DataTableColumnHeader } from "@/components/ui/data-table-column-header";

import { TDeleteDialog } from "@/main/common/components/TForm/delete/TDeleteDialog";
import { UnitFormDialog } from "../blocks/dialogs/UnitFormDialog";

import { GetCity } from "@/main/global/api/restful/userManagmentAPI/CityManager/CityTypes";
import { useDeleteCity } from "@/main/global/api/restful/userManagmentAPI/CityManager/CityQuery";

export const useColumns = ({
  type = "",
  refetch,
}: {
  type: string | undefined;
  refetch: any;
}) => {
  const { t } = useTranslation("cities");
  console.log(type);
  const { mutate: mutate } = useDeleteCity();
  const columns: ColumnDef<GetCity>[] = [
    {
      id: "englishName",
      accessorKey: "englishName",
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          className="!text-start justify-start"
          title={t("cities.table.header.englishName")}
        />
      ),
      cell: ({ row }) => (
        <div className=" justify-start text-start mx-3">
          {row.original.name}
        </div>
      ),
      meta: {
        header: t("cities.table.header.englishName"),
      },
    },
    {
      id: "arabicName",
      accessorKey: "arabicName",
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("cities.table.header.arabicName")}
        />
      ),
      cell: ({ row }) => row.original.nameAr,
      meta: {
        header: t("cities.table.header.arabicName"),
      },
    },

    {
      id: "actions",
      header: t("cars.table.header.actions"),
      cell: ({ row }) => (
        <div className="flex justify-center items-center gap-2">
          <div className="p-0 w-[43px] h-[36px] hover:opacity-70 bg-[#E9EDE9] flex items-center justify-center rounded-md">
            <UnitFormDialog
              refetch={refetch}
              type={"edit"}
              btn
              unit={row.original}
            />{" "}
          </div>

          <TDeleteDialog
            id={row.original?.id || ""}
            mutate={mutate}
            isAllow={true}
          />
        </div>
      ),
      meta: {
        header: t("cars.table.header.actions"),
      },
    },
  ];

  return {
    columns,
  };
};
