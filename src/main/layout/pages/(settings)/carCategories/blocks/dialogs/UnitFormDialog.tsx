      ...
      {type !== "delete" && (
        <Dialog open={open} onOpenChange={setOpen} modal={false}>
          <DialogTrigger asChild>
            <TTriggerForm
              type={type}
              addText={t("Categories.form.addText")}
              btn={btn}
            />
          </DialogTrigger>
          <DialogContent
                ...
                <Form {...form}>
                  ...
                  >
                    <div className="flex flex-col md:flex-1 gap-y-3 ">
                      <TFormField
                        placeholder={t("Categories.form.placeholder.name_en")}
                        typeField="input"
                        form={form}
                        name="englishName"
                        label={t("Categories.form.name_en")}
                        readOnly={type === "view"}
                      />{" "}
                      <div className=" mt-[24px]">
                        <TFormField
                          placeholder={t("Categories.form.placeholder.name_ar")}
                          typeField="input"
                          form={form}
                          name="arabicName"
                          label={t("Categories.form.name_ar")}
                          readOnly={type === "view"}
                        />
                      </div>{" "}
                      <div className=" mt-[24px]">
                        <TFormField
                          placeholder={t(
                            "Categories.form.placeholder.deliveryPrice"
                          )}
                          type="number"
                          typeField="input"
                          form={form}
                          name="deliveryPrice"
                          label={t("Categories.form.deliveryPrice")}
                          readOnly={type === "view"}
                        />
                      </div>{" "}
                      <div className=" mt-[24px]">
                        {/* <TextEditor
                          form={form}
                          name="notes"
                          placeholder={t("Categories.form.placeholder.notes")}
                          label={t("Categories.form.notes")}
                          readOnly={type === "view"}
                        /> */}
                      </div>
                    </div>
