...
export function UnitFormDialog({ unit, type, btn = true }: Props) {
  ...
  const { i18n, t } = useTranslation("carEngines");
      ...
      {type !== "delete" && (
        <Dialog open={open} onOpenChange={setOpen} modal={false}>
          <DialogTrigger asChild>
            <TTriggerForm
              type={type}
              addText={t("carEngines.form.addText")}
              btn={btn}
            />
          </DialogTrigger>
          <DialogContent
                ...
                <Form {...form}>
                  ...
                  >
                    <div className="flex flex-col md:flex-1 gap-y-3 ">
                      <TFormField
                        placeholder={t("carEngines.form.placeholder.name_en")}
                        typeField="input"
                        form={form}
                        name="englishName"
                        label={"اسم المحرك"}
                      ...
                      <div className=" mt-[24px]">
                        <TFormField
                          placeholder={t("carEngines.form.placeholder.name_ar")}
                          typeField="input"
                          form={form}
                          name="arabicName"
                          label={"اسم المحرك بالعربية"}
                          readOnly={type === "view"}
                        />
                      </div>{" "}
                      <div className=" mt-[24px]">
                        {/* <TextEditor
                          form={form}
                          name="notes"
                          placeholder={t("carEngines.form.placeholder.notes")}
                          label={t("carEngines.form.notes")}
                          readOnly={type === "view"}
                        /> */}
                      </div>
                    </div>
