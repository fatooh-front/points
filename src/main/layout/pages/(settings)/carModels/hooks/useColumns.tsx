...
import { GetCarsModels } from "@/main/global/api/restful/userManagmentAPI/carsModelsManager/carsModelsTypes";

import { TDeleteDialog } from "@/main/common/components/TForm/delete/TDeleteDialog";
import { useDeleteCarsModels } from "@/main/global/api/restful/userManagmentAPI/carsModelsManager/carsModelsQuery";
...
export const useColumns = ({ type = "" }: { type: string | undefined }) => {
  ...
  console.log(type);
  const { mutate: mutate } = useDeleteCarsModels();
  const columns: ColumnDef<GetCarsModels>[] = [
    {
      id: "englishName",
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          ...
          title={t("carModel.table.header.englishName")}
        />
      ),
      cell: ({ row }) => (
      ...
      meta: {
        header: t("carModel.table.header.englishName"),
      },
    },
    {
      id: "arabicName",
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("carModel.table.header.arabicName")}
        />
      ),
      cell: ({ row }) => row.original.arabicName,
      meta: {
        header: t("carModel.table.header.arabicName"),
      },
    },
