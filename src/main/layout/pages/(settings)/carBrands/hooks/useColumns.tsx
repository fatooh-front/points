...
import { GetBrand } from "@/main/global/api/restful/userManagmentAPI/BrandManager/BrandTypes";
...
export const useColumns = ({ type = "" }: { type: string | undefined }) => {
  ...
  const columns: ColumnDef<GetBrand>[] = [
    {
      id: "englishName",
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          ...
          title={t("carBrands.table.header.englishName")}
        />
      ),
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("carBrands.table.header.arabicName")}
        />
      ),
