...
export default function Maseges() {
  ...
  return (
    <div className=" p-6 w-full  h-full bg-white">
      <div className=" justify-end h-full rounded-[8px]  flex flex-col items-start py-4 ">
        {demoMessages.map((message) => (
          ...
          >
            ...
            <div
              className={` ${
                message.isme ? "justify flex-row-reverse gap-2 " : ""
              } min-w-[100px] mt-2 flex items-center  text-right rounded-[12px] p-[8px] bg-[#F1F1F1]`}
            >
              <p className="text-[#23262F]  font-normal">{message.content}</p>{" "}
              <span className="text-[#7E858E] ms-[10px]   justify-start text-xs font-normal">
                {message.time}
              </span>
            </div>
            <div
